/* Multi-workgroup tensor S2G descriptor-mix kernel. */

#define SHARED_ELEMS 64
#define DESC_BASE_BYTES 1024u

kernel void
multi_wg_tma_s2g_descriptor_mix(__global const uint *desc_guard,
                                __global uint *desc,
                                ...
                                __global uchar *dst)
{
  ...
  for (uint i = (uint)lid; i < SHARED_ELEMS; i += get_local_size(0)) {
    shared_buf[i] = 0xC2300000u + gid * 0x1000u + i;
  }
  if (lid == 0 && desc_guard[0] == 0x2468ace0u) {
  ...
  if (lid == 0) {
    ...
    __asm__ volatile(
      ...
      "add  %[desc], %[desc], %[base]\n\t"
      "csrr %[desc_off], 0x808\n\t"
      "slli %[dst_off], %[desc_off], 8\n\t"
      "slli %[desc_off], %[desc_off], 7\n\t"
      ...
      : [desc] "=&r"(desc_ptr), [dst] "=&r"(dst_ptr),
        [desc_off] "=&r"(desc_offset), [dst_off] "=&r"(dst_offset)
