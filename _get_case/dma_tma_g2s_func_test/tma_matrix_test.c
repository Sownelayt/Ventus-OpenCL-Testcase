...
static const char *g_functional_cases[] = {
  "FP32_2D_4x4_full",
  "FP32_2D_subbox_8x8_at_2_2",
  "FP32_2D_padded_rows_4x4_stride64",
  "FP32_3D_interleave16_C6_W3_N2",
  "FP16_2D_8x4",
  "I8_2D_16x4",
  "FP32_2D_estride2_cols",
  "FP32_2D_swizzle64_rows",
  "FP32_2D_oob_zero_dim0",
  "FP16_2D_oob_fill",
  "FP32_2D_oob_estride_dim1",
};

static const tma_config_t *
find_case_exact(const char *name)
{
  for (size_t i = 0; i < g_num_cases; i++) {
    if (strcmp(g_cases[i].name, name) == 0) return &g_cases[i];
  }
  return NULL;
}
...
{
  char cmd[2048];
#ifdef TMA_MATRIX_EMBEDDED
  int n = snprintf(cmd, sizeof(cmd), "\"%s\" --tma-matrix-case \"%s\"",
                   self, case_name);
#else
...
static int
run_case_list_processes(const char *self, const char *summary_name,
                        const char *const *case_names, size_t case_count)
{
  size_t total_pass = 0, total_fail = 0, total_skip = 0;

  for (size_t i = 0; i < case_count; i++) {
    const tma_config_t *c = find_case_exact(case_names[i]);
    if (!c) {
      printf("[%2zu/%zu] %-36s\n", i + 1, case_count, case_names[i]);
      printf("      FAIL: case not found\n");
      total_fail++;
      continue;
    }

    printf("[%2zu/%zu] %-36s dtype=%-5s rank=%u boxBytes=%zu\n",
           i + 1, case_count, c->name,
    ...
    fflush(stdout);
    int rc = run_case_process(self, c->name);
    if (rc == 0) {
      printf("      PASS\n");
      total_pass++;
    } else {
...
static int
parse_case_index(const char *text, size_t *out)
{
  char *end = NULL;
  unsigned long v = strtoul(text, &end, 10);
  if (!text[0] || (end && *end) || v == 0) return 1;
  *out = (size_t)v;
  return 0;
}
...
usage(const char *prog)
{
  fprintf(stderr,
          "Usage:\n"
          "  %s\n"
          "  %s <case-name-substring>\n"
          "  %s --functional-suite\n"
          "  %s --range <first-1based> <last-1based|end>\n"
          "  %s --list-cases\n",
...
main(int argc, char **argv)
{
  ...
  if (argc >= 2 && strcmp(argv[1], "--functional-suite") == 0) {
    return run_case_list_processes(
        argv[0], "TMA matrix functional",
        g_functional_cases,
        sizeof(g_functional_cases) / sizeof(g_functional_cases[0]));
  }

  if (argc >= 2 && strcmp(argv[1], "--range") == 0) {
    size_t first = 0;
    size_t last = 0;
    if (argc != 4 || parse_case_index(argv[2], &first)) {
      usage(argv[0]);
      return 2;
    }
    if (strcmp(argv[3], "end") == 0) {
      last = g_num_cases;
    } else if (parse_case_index(argv[3], &last)) {
      usage(argv[0]);
      return 2;
    }
    if (first > g_num_cases || first > last) {
      usage(argv[0]);
      return 2;
    }
    if (last > g_num_cases) last = g_num_cases;
  ...
  if (total_fail == 0 && (total_pass > 0 || selected_count > 0)) printf("OK\n");
  else printf("FAILED\n");
...
FINISH:
  ...
  return (total_fail == 0 && (total_pass > 0 || selected_count > 0)) ? 0 : 1;
}
